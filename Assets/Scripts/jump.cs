...
public class jump : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (state == 2)
        {
            ...
            {
                float nextZ = (euler.z >= 360 - fix || euler.z <= 180) ? euler.z - 5f * Time.deltaTime : euler.z;
                transform.eulerAngles = new Vector3(euler.x, euler.y, nextZ);
                rbody.AddTorque(0, torque, 0);
            }
            if (Input.GetKey(KeyCode.LeftArrow))
            {
                float nextZ = (euler.z >= 180 || euler.z <= fix) ? euler.z + 5f * Time.deltaTime : euler.z;
                transform.eulerAngles = new Vector3(euler.x, euler.y, nextZ);
                rbody.AddTorque(0, -torque, 0);
            }
            ...
            {
                rbody.velocity = nv;
                rbody.AddRelativeForce(500 * Vector3.forward * Time.deltaTime);
            }
            ...
            if (Input.GetKey(KeyCode.UpArrow))
            {
                ...
                transform.eulerAngles = new Vector3(eu.x + 40 * Time.deltaTime, eu.y, eu.z);
            }
            else
            {
                Vector3 eu = transform.eulerAngles;
                transform.eulerAngles = new Vector3(eu.x + 20 * Time.deltaTime, eu.y, eu.z);
            }
        }
        if (state == 3)
        {
